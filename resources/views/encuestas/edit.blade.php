...
@section('content')
	<div class="row">
		<div class="browser-window">
			<div class="top-bar">
	          <h4>Editar - {{$dbData->descripcion}}</h4>
	        </div>
	        <div class="content">
				<form class="col s12" id="realForm" action="{{route('encuestas.update', $dbData->id)}}" method="POST">
					<div class="row">
					...
					<div class="row">
						<div class="input-field col s6">
							<input type="date" class="datepicker" id="fec_ini" name="fec_ini"/>
							<label for="fec_ini">Fecha Inicial</label>
						</div>
						<div class="input-field col s6 ">
							<input type="date" class="datepicker" id="fec_fin" name= "fec_fin"/>
					...
					<div class="row">
						...
						<div class="input-field col s4">
							<textarea id="numero" type="text" class="materialize-textarea" name="numero"></textarea> 
					...
					<div class="row">
						<div class="input-field col s4">
							<p>
								<input  type="checkbox" id="abierta" name="abierta"/>
						...
						<div class="input-field col s4">
							<p>
								<select id="control" name="control">
									<option value="R">Selección Múltiple</option>
									<option value="C">Casilla de Verificación</option>
								</select>
						...
						<div class="input-field col s4" id="opcion-div">
							<p>
								<select id="escalas">
									@foreach($dbEscalas as $key=>$value)
					...
					<div class="row">
						<div class="browser-window">
			                ...
			                <div class="content">
			                	<table id="optlist" class="highlight">
			                		<thead>
				                      <tr>
					...
					<input id="fields" type="hidden" name="fields" value="{{$dbFields}}">
					<input id="options" type="hidden" name="options">
					<input type="hidden" name="_token" value="{{{ csrf_token() }}}" />
					{{ method_field('PUT') }}
					<div class="button-group">
					<a class="btn waves-effect waves-light amber" href="#" id="add">Pregunta
    					<i class="material-icons left">add</i>
      				</a>
					<a class="btn waves-effect waves-light light-blue" href="#" id="preview">Vista Previa
			                	...
			                	<table id="detlist" class="highlight">
			                		<thead>
				                      <tr>
				                      	 ...
				                      	 <th style="width:20%;">Abierta</th>
										 <th style="width:30%">Opción</th>
				                      </tr>
				                    </thead>
				                    <tbody>
				                    	@foreach($dbData->detalleEncuesta as $key=>$detalle)
											<tr class="ingData" id="ingRow{{$key+1}}">
			            						<td style="width: 10%">
			            							<div id="numero{{$key + 1}}">
			            								{{$detalle->nro_pregunta}}
			            							</div>
			            						<td style="width:40%">
			            							<div id="pregunta{{$key + 1}}">
			            								{{$detalle->pregunta}}
			            							</div>
			            						</td>
			            						<td style="width:20%">
			            							<div id="abierta{{$key + 1}}"> 
			            								@if($detalle->abierta == "S")
			            									Sí
			            								@else
															No
			            								@endif
			            						...
			            						<td style="width:15%;">
			            							<button class="btn waves-effect waves-light amber " onClick="editItem({{$key + 1}})" type="button">Editar</button>
			            						</td>
			            						<td style="width:15%;">
			                						<button class="btn waves-effect waves-light red" onClick="removeItem({{$key + 1}})" type="button">Borrar</button>
			            						</td>
	...
	<script>
		...
		$('#realForm').keypress(function(event){
  		...
  		$(document).ready(function() {
  			...
  			var fecIni = $('#fec_ini').pickadate('picker');
  			fecIni.set('select', '{{$dbData->fecha_vigencia_ini->format('Y-m-d')}}', {format: 'yyyy-mm-dd'});
  			var fecFin = $('#fec_fin').pickadate('picker');
  			fecFin.set('select', '{{$dbData->fecha_vigencia_fin->format('Y-m-d')}}', {format: 'yyyy-mm-dd'});

  			try{
  				item = JSON.parse($('#fields').val());
  			} catch (e){
  				console.error(e);
  			}
			...
			if(!$("#pregunta").val() == ''){
			    ...
			    var outHTML =
			        '<tr class="ingData" id="ingRow' + rowCount + '">'
			            + '<td style="width:40%"><div id="numero' + rowCount + '">' + numero + '</div></td>'
